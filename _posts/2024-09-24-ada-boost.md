...
trong đó, ta sẽ tạm đặt $w^{(m)}i = e^{-y{i}f_{m-1}(x_i)}$ vì cụm này chỉ là kết quả của mọi learners trước nên ta sẽ xem là một biến chưa thể can thiệp. Như vậy, ta sẽ có
